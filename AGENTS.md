...
- 员工口径：采集走「招标采集员」外壳（crawl/collector_employee.py，implements collector/v1.2.0）→ 契约 output（含 tenderFile，可空）+ reports/collector-report.json（7 项指标）。实体升维走「实体情报员」外壳（crawl/intel_employee.py，implements entity_intel/v1.0.0）→ 读 notices 累积库产出实体图（entities/winners/relations，含中标企业抽取与周期粗估）+ reports/intel-report.json + handoffs/intel/latest.json；入口 scripts/intel_run.py，先跑 scripts/migrate_ops.py 建 notices.winner 列。采集员详情补全（agent 纵向链）编排在 `crawl/detail_pass.py`：**已接进采集轮**（`collector_employee.run()` 列表采集后跑一个有界阶段，默认单轮 ≤80 条 / 每站 ≤10 条 / 限时 900s，配置 `config/anti_bot.json` 的 `detail_pass`，报告字段 `detailPass`；`SPIDER_NO_DETAIL_PASS=1` 关、`SPIDER_DETAIL_AI=1` 开 AI），手动大批量入口 `scripts/collector_detail.py`（不限时）。链路 = 详情 → 原发寻址 → 原发转爬 + AI 字段抽取兜底（写 `notices.summary`/`tenderfile_path`/`ai_fields`；AI 用 `crawl/ai_extract.py`，DeepSeek 文本、失败降级规则、只抽取不判断）。**为什么必须进采集轮**：此前这条链没有任何定时入口（本机无权新建计划任务），实测 3166 条里 3103 条 `detail_status` 为 null、仅 7 条落附件正文 —— 下游解析员直接断粮（2026-09-19 修复）。同一轮修复：`SUMMARY_CHARS` 200→2000（bridge/http 源摘要实测全被截在 200 字）、ccgp 字段分支不再跳过附件链。**采集员入口自带启动自记录**（`scripts/collector_run.py`，2026-09-19）：stdout/stderr 全量落 `logs/collector_run_<时间戳>.log`（留最近 30 份）+ 启动戳 `logs/collector_launch.json` + 未捕获异常留栈并 exit 1；`SPIDER_NO_RUN_LOG=1` 关。原因：任务动作是裸 `python.exe scripts\collector_run.py`，Task Scheduler 不落 stdout、本机 `TaskScheduler/Operational` 日志亦关闭，启动期死亡否则完全无痕（09-18 22:00 实证）。
- WebBridge 开桥 = 已固化的代码能力，禁止重新逆向：桥服务端 = **官方 daemon `~/.kimi-webbridge/bin/kimi-webbridge.exe`**（127.0.0.1:10086，Kimi 扩展自动连；**旧 `scripts/webbridge_server.py` 已弃用，不得再让它占 10086** —— 它一占，官方 daemon 起不来、扩展连不上，WebBridge 源就静默 0 条）。可用性严格判定：`wb.available()` = daemon 在线 **且** `extension_connected=true`（只看端口会误报）。**浏览器统一走 Chrome（Edge 已弃用）**：开页面只用 `webbridge_client.open_in_chrome()`（Chrome 候选路径；`EDGE_CANDIDATES` 已删），**禁用 `webbrowser.open`**（走系统默认浏览器，可能落到 Edge）。保活三层：常驻 `scripts/wb_bridge.py watch`（Windows 任务 SpidermanWebBridge 的动作，每 120s 巡检 + 写心跳 `data/web/wb_watch_heartbeat.json`）+ `wb_bridge.py ensure-daemon`（心跳陈旧则后台重拉）+ 采集时 `_wait_bridge_ready` 等桥恢复（默认 300s，`SPIDER_BRIDGE_WAIT_SEC` 可调；HTTP 源不受影响）。协议文档 .cursor/docs/webbridge_bridge.md。**标签卫生（2026-09-19，用户报 Chrome 内存越用越多）**：桥给我们开的每个 tab 都必须自己关，且**不能依赖「进程还活着」**——详情抓取的会话名 `bridge_session_name()`（`tf-<站>-<md5(url)[:8]>`）**登记落盘** `data/web/wb_open_sessions.json`；`fetch_detail_via_bridge`/`fetch_cebpub_via_bridge` 单条抓完在 `finally` 里立即 release 自己的会话（峰值 tab 从一轮几十个降到 1）；采集轮收尾再兜底 `close_bridge_tabs()`；手工 `python scripts/wb_bridge.py clean-tabs [--stale-min N]`，常驻 watch 每 10 轮（约 20 分钟）自动清一次「登记 >90 分钟」的陈旧会话。**注意 `list_tabs` 是按 session 隔离的**：查默认 session 得到 0 个 tab 是假象，必须知道会话名才看得到。
- 反爬兜底原则：部分结果必须保留（SourceError.partial）；频控靠冷却阶梯不靠轰炸；测试夹具必须自清理。**桥爬虫（jiangsu/qianlima）逐词增量落库**——进程被外部强杀也不丢已采，异常退出如实报 `partial` + 真实条数 + 已采 notices（2026-09-19 实证：原实现整轮词循环末尾才 upsert，进程被杀导致 29 个词颗粒无收、江苏当天 0 条入库）。**台账 running 行不会永久「卡死」**：每轮采集开头 `crawl.db_store.sweep_orphan_runs(max_age_hours=3)` 全局闭环孤儿行并写入报告 `orphanRuns`（被杀时 `finish_run` 永不执行）。**入口与保活都必须自记录日志**（共用 `crawl/run_log.py`）：`logs/collector_run_<时间戳>.log`（留 30 份）+ `logs/collector_launch.json` 启动戳 + `logs/wb_watch.log`（保活，>2MB 轮转），未捕获异常留栈并 exit 1；`SPIDER_NO_RUN_LOG=1` 关、`SPIDER_LOG_DIR` 可重定向。原因：Task Scheduler 不落 stdout 且本机 `TaskScheduler/Operational` 关闭，`ensure_daemon` 还把保活子进程输出指向 DEVNULL —— 不自己记录就必然在黑盒里死（09-18 22:00、09-19 00:38 两次实证）。**浏览器源必须与 HTTP 内核同过滤口径**：8 城白名单 + 发布时间窗（无日期不丢），共用 `crawl/source_filter.py::filter_notices` —— 浏览器源是各自模块直接 upsert 的，过滤得自己实现；tgnet 原先**完全没有过滤**，实测 385 条里 340 条早于窗口、319 条不在 8 城（2026-09-19 已修，保留启用只收窗）。
