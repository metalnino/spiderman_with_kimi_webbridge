...
- 架构：HTTP 主爬 + 真浏览器（WebBridge/Playwright）过反爬；NAS 已弃用（2026-08-20），本机为唯一运行点、十站全开：ccgp/chinabidding/ggzy/jsggzy/yfbzb/qianlima/rccchina=HTTP，cebpub/tgnet=Playwright，jiangsu=WebBridge；共享同一 MySQL。Windows 任务 SpidermanCollector（11:00/22:00，config 驱动 keywords.json+platforms.json）自动覆盖全部站点与 20 词。
