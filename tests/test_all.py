...
class TestLedgerAPI(unittest.TestCase):
    ...
    def test_meta_sources_pruned(self):
        ...
        # 九站运行（HTTP + Playwright + WebBridge）；rccchina 注册墙暂缓，不应出现
        for sid in ("cebpub", "chinabidding", "ccgp", "ggzy", "jsggzy",
                    "jiangsu_zhaobiao", "yfbzb", "qianlima", "tgnet"):
            self.assertIn(sid, body["sources"])
        self.assertNotIn("rccchina", body["sources"])
